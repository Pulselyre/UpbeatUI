...
For example, the following will take half of the available height, betwee half and 90% of available width, be positioned dynamically based on the bound `XPosition` and `YPosition` properites on the _ViewModel_, and will be kept within bounds (i.e., it will be rendered completely within the available space rather than potentially having a portion outside).
